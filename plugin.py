...
class dnmlb(callbacks.Plugin):
    ...
    def __init__(self, irc):
        ...
        self.inprogress_status = [
            'Delayed Start', 'In Progress', 'Manager Challenge', 'Suspended', 'Review']
    ...
    def _searchlist(self, wmatch, wlist):
        #self.log.info(
        #    '_searchlist: called with word: %s list: %s', wmatch, wlist)
        for word in wlist:
            if word == wmatch:
                self.log.info(
                    '_searchlist: matched word: %s in list: %s', word, wlist)
                return word
                break
        #self.log.info(
        #    '_searchlist: no match for word: %s in list: %s', wmatch, wlist)
        return False
    ...
    def _getscoreboardinfo(self, date):
        ...
        def __buildhrs(team_code, hr_data):
            ...
            for player in hrs:
                ...
                yr_hr = player['std_hr']
                player_item = "%s%s (%s)" % (
                    name, '[' + hr + ']' if int(hr) > 1 else '', yr_hr)
      ...
      """
        ...
        try:
            scoreboard_json = self._fetchurl('scoreboard', date).decode()
        except:
            #irc.reply('Error fetching data')
            return 'Error fetching data'
        # pprint.pprint(scoreboard_json)

        # Bail if we can't get our scoreboard.
        if 'error_' in scoreboard_json:
            error = "ERROR: Couldn't get scoreboard! Code: %s" % (
                scoreboard_json)
        ...
        if type(games) == dict:
            ...
            self.log.info(
                '_getscoreboardinfo: games is an unknown type: %s' % (type(games)))
            return 'ERROR: Issue parsing scoreboard info'

        numgames = len(games)
        #self.log.info(
        #    '_getscoreboardinfo: numgames: %s date: %s', numgames, date)
        ...
        if numgames > 0:
            #self.log.info(
            #    '_getscoreboardinfo: Found %i Game(s) for %s', numgames, date)
            all_games = {}
            all_gids = []
            ...
            for game in games:
                ...
                #self.log.info(
                #    '_getscoreboardinfo: gid %s status %s', gid, status)
                ...
                if status in self.upcoming_status:
                    ...
                    if 'broadcast' in game:
                        if 'tv' in game['broadcast']['away']:
                            away_tv_feed = __buildtv(
                                game['broadcast']['away']['tv'])
                        else:
                            away_tv_feed = '--'
                        if 'tv' in game['broadcast']['home']:
                            home_tv_feed = __buildtv(
                                game['broadcast']['home']['tv'])
                        else:
                            home_tv_feed = '--'
                ...
                # {{{ Handle games in inprogress_status.
                    ...
                    all_games[gid]['runners'] = __buildrunners(
                    ...
                    all_games[gid]['last_play'] = game['pbp']['last'].replace(
                        '  ', ' ').replace('.  ', '. ')
                ...
                # {{{ Handle games in final_status.
                    ...
                    # Homeruns.
                    all_games[gid]['away_hrs'] = {
                        'total': game['linescore']['hr']['away']}
                    all_games[gid]['home_hrs'] = {
                        'total': game['linescore']['hr']['home']}
                    if int(all_games[gid]['away_hrs']['total']) > 0:
                        all_games[gid]['away_hrs']['players'] = __buildhrs(
                            game['away_code'], game['home_runs']['player'])
                    if int(all_games[gid]['home_hrs']['total']) > 0:
                        all_games[gid]['home_hrs']['players'] = __buildhrs(
                            game['home_code'], game['home_runs']['player'])
                    # Pitchers.
                ...
                # }}}
                    ...
                    error = "ERROR: Unknown gamestatus for game %s: %s" % (
                        gid, status)
                    return error
    ...
    def _procsearchresult(self, results, games):
        ...
        if results:
            for gid in results:
                ...
                status = game['status']
                #self.log.info(
                #    '_procsearchresult: gid: %s status: %s', gid, status)
                if status in self.upcoming_status:
                    ...
                    gameresults.append(
                        "ERROR: gameid %s has unknown status %s" % (gid, status))
        else:
    ...
    def _printscore_all(self, all_games):
        #self.log.info('_printscore_all: called with datatype %s',
        #              type(all_games))
        # pprint.pprint(all_games)
        #sorted_games = sorted(all_games['gids'], key=lambda k: k['status'])
        if (type(all_games) != dict):
            ...
            output_line2 = []
            counter = 0
            #all_games = sorted(all_games, key=lambda k: k['gids']['status'])
            sorted_games = []
            for gid in all_games['gids']:
                if all_games[gid]['status'] != 'Final' and all_games[gid]['status'] != 'Cancelled' and all_games[gid]['status'] != 'Postponed' and all_games[gid]['status'] != 'Game Over':
                    sorted_games.append(all_games[gid])
            for gid in all_games['gids']:
                if all_games[gid]['status'] == 'Final' or all_games[gid]['status'] == 'Cancelled' or all_games[gid]['status'] == 'Postponed' or all_games[gid]['status'] == 'Game Over':
                    sorted_games.append(all_games[gid])
            #sorted_games = sorted(sorted_games, key=lambda k: k['status_ind'])
            # for g in sorted_games:
            #    if g['status_ind'] == 'F':
            #        sorted_games += sorted_games.pop(g)
            # print(type(sorted_games))
            # pprint.pprint(sorted_games)
                ...
                if (status in self.final_status) or (status in self.inprogress_status):
                    ...
                    if (status in self.final_status):
                        ...
                        if (status == 'Suspended'):
                            ...
                            if 'reason' in game:
                                inning_state = '%s (%s)' % (
                                    inning_state, game['reason'])
                        else:
                            #print(game['inning_state'][0])
                            inning_state = game['inning_state'][0] + \
                                game['inning']
                            if (game['inning_state'][0] == 'M'):
                                inning_state = self._blue(inning_state)
                            elif (game['inning_state'][0] == 'E'):
                                inning_state = self._orange(inning_state)
                            else:
                    ...
                    if 'delay_reason' in game:
                        line = "%s @ %s %s (%s) |" % (
                            away, home, self._yellow('DLY'), game['delay_reason'])
                        ...
                        if (game['inning_state'][0] == 'M'):
                            ...
                            inning_state = self._green(inning_state)
                        status = "%s %s (%s)" % (
                            inning_state, self._yellow('DLY'), game['reason'])
                        line = "%s %s %s %s %s |" % (
                            away, away_score, home, home_score, status)
                    else:
                        if (status == 'Cancelled'):
                            line = "%s @ %s %s |" % (
                                away, home, self._red('CAN'))
                        else:
                            line = "%s @ %s %s |" % (
                                away, home, self._red('PPD'))
                else:
    ...
    def _printscore_upcoming(self, game):
        ...
        team_line = "%s (%s) @ %s (%s)" % (
            away, game['away_team_wl'], home, game['home_team_wl'])
        ...
        if ap['name'] == '':
            ...
            away_pitcher = "%s %s (%s/%s)" % (ap['name'],
                                              ap['hand'], ap['era'], ap['wl'])
        if hp['name'] == '':
            home_pitcher = 'TBD'
        else:
            home_pitcher = "%s %s (%s/%s)" % (hp['name'],
                                              hp['hand'], hp['era'], hp['wl'])
        ...
        if status == 'Warmup':
            if pitcher_line:
                ...
                status_line = "%s %s :: %s :: %s" % (
                    team_line, game['time'], pitcher_line, status)
            else:
                status_line = "%s %s :: %s" % (team_line, game['time'], status)
        else:
            if pitcher_line:
                # NY Mets @ Atlanta 7:10PM :: Gee (-.--/0-0) vs Teheran (1.50/1-0) :: TV: SNY/FSSO
                status_line = "%s %s :: %s :: TV: %s" % (
                    team_line, game['time'], pitcher_line, tv_feed)
            else:
                status_line = "%s %s :: TV: %s" % (
                    team_line, game['time'], tv_feed)
    ...
    def _printscore_inprogress(self, game):
        ...
        if (status != 'Delayed Start'):
            ...
            if (inning_state == 'M'):
                inning = self._blue(inning)
            elif (inning_state == 'E'):
                inning = self._orange(inning)
            else:
                # def _fetchurl(self, utype, date=False, gid=False):
                rawbox_xml = self._fetchurl(
                    'rawbox', game['date'], gid).decode()
                pitcher_data = {}
                if 'error_' in rawbox_xml:
                    self.log.info(
                        '_printscore_inprogress: error fetching rawbox %s', rawbox_xml)
                else:
                    ...
                    for p in rawbox_xml.xpath('//team//pitcher'):
                        ...
                        pitcher_data[p_id] = {
                            'era': p.attrib['bam_era'], 'pitches': p.attrib['np']}
        else:
            inning_state = None
            inning = None
        ...
        if (status == 'Suspended'):
            score_line = "%s %s %s %s %s %s (%s)" % (
                away, away_score, home, home_score, inning, self._red('Suspended'), game['reason'])
        elif (status == 'Delayed Start'):
            score_line = "%s %s %s %s %s (%s)" % (
                away, away_score, home, home_score, self._yellow('Delayed'), game['reason'])
        else:
            score_line = "%s %s %s %s %s" % (
                away, away_score, home, home_score, inning)
        # B:1 S:2 O:2
        count_line = "%s%s %s%s %s%s" % (self._greenpad('B:'), game['balls'], self._redpad(
            'S:'), game['strikes'], self._yellowpad('O:'), game['outs'])
        ...
        if inning_state:
            if inning_state in ['M', 'E']:
                ...
                if pitcher_data:
                    ...
                    play_line = "%s %s %s %s (%s)" % (
                        ab, game['batting'], cp, game['pitching'], pitch_count)
                else:
                    play_line = "%s %s %s %s" % (
                        ab, game['batting'], cp, game['pitching'])
                ...
                status_line = "%s :: %s :: %s" % (
                    score_line, count_line, info_line)
        else:
    ...
    def _printscore_final(self, game):
        ...
        if 'players' in game['away_hrs']:
            ...
            away_team_hr_line = "%s(%s): %s" % (
                self._stripf(away), ahr['total'], ahrplayers)
        else:
        ...
        if 'players' in game['home_hrs']:
            ...
            home_team_hr_line = "%s(%s): %s" % (
                self._stripf(home), hhr['total'], hhrplayers)
        else:
        ...
        if sp_line:
            pitcher_line = "%s %s %s %s %s %s" % (self._green(
                'W:'), wp_line, self._red('L:'), lp_line, self._yellow('SV:'), sp_line)
        else:
            # W: Kelly, J (1.29/1-0) L: Warren (1.69/0-1)
            pitcher_line = "%s %s %s %s" % (self._green(
                'W:'), wp_line, self._red('L:'), lp_line)
        ...
        if hr_line:
            ...
            status_line = "%s :: %s :: %s" % (
                score_line, pitcher_line, hr_line)
            if reason:
    ...
    def _printscore_postponed(self, game):
        ...
        status_line = "%s (%s) @ %s (%s) %s %s" % (
            away, arec, home, hrec, game['time'], self._red(game['status']))
        if 'reason' in game:
        ...
        if (game['status_ind'] == 'IR'):
            ...
            inning = "%s %s (%s)" % (
                inning, self._yellow('Delayed'), game['reason'])
            ...
            score_line = "%s %s %s %s %s" % (
                away, away_score, home, home_score, inning)
            count_line = "%s%s %s%s %s%s" % (self._greenpad(
                'B:'), b, self._redpad('S:'), s, self._yellowpad('O:'), o)
            ...
            if (inning_state == 'M') or (inning_state == 'E'):
                ...
                status_line = "%s :: %s :: %s" % (
                    score_line, count_line, batter_line)
                if lp:
                    status_line = "%s :: %s :: %s :: %s" % (
                        score_line, count_line, batter_line, lp)
    ...
    def _getstandings(self, league, division=False):
        ...
        def __returnstandings(standings_html, league, division):
            ...
            if (league == 'al'):
                page_path = '//section[@id="league-103"]/table/tbody'
            if (league == 'nl'):
                page_path = '//section[@id="league-104"]/table/tbody'
            ...
            if (league == 'nl'):
                league = 'NL'
            if (league == 'NL') and (division == 'east'):
                division = 'Least'
            if (division == 'east'):
                division = 'East'
            if (division == 'central'):
            ...
            if (playoffs):
                banner = "%-16s %-5s %-5s %-5s %-5s %-5s %-5s %-4s" % (
                    title, 'W', 'L', 'PCT', 'GB', 'WCGB', 'L10', 'STRK')
            else:
                banner = "%-16s %-5s %-5s %-5s %-5s %-5s %-4s" % (
                    title, 'W', 'L', 'PCT', 'GB', 'L10', 'STRK')

            banner = self._buline(banner)
            output.append(banner)

            div_data = standings_html.xpath(page_path)[num]
            div_teams = div_data.cssselect(
                'td.standings-col-division span.title')
            ...
            while num_teams > counter:
                ...
                if ('W' in strk):
                    strk = self._green(strk)
                if ('L' in strk):
                    strk = self._red(strk)

                if (not playoffs and gb == '-'):
                    team = self._bold(team)
                    line = "%-18s %-5s %-5s %-5s %-5s %-5s %-5s" % (
                        team, wins, losses, pct, gb, l10, strk)
                # Hunt for october, start showing wild card contenders.
                elif (playoffs and gb == '-'):
                    team = self._bold(team)
                    line = "%-18s %-5s %-5s %-5s %-5s %-5s %-5s %-5s" % (
                        team, wins, losses, pct, gb, wcgb, l10, strk)
                    ...
                    line = "%-16s %-5s %-5s %-5s %-5s %-5s %-5s %-5s" % (
                        team, wins, losses, pct, gb, wcgb, l10, strk)
                else:
                    line = "%-16s %-5s %-5s %-5s %-5s %-5s %-5s" % (
                        team, wins, losses, pct, gb, l10, strk)
        ...
        #self.log.info(
        #    '_getstandings called. league: %s division: %s', league, division)
        standings_html = self._fetchurl('standings').decode()

        if 'error_' in standings_html:
            error = "ERROR: Couldn't get standings! Code: %s" % (
                standings_html)
    ...
    def _parseargsmlb(self, args):
        ...
        if (type(args) == str):
            #self.log.info(
            #    '_parseargs: called with no arguments, defaulting to all')
            arg1 = 'all'
        elif (len(args) == 1):
            #self.log.info('_parseargs: called with 1 argument: %s', args[0])
            arg1 = str(args[0]).replace('-', '').lower()
            arg2 = None
        elif (len(args) == 2):
            #self.log.info(
            #    '_parseargs: called with 2 arguments: %s %s', args[0], args[1])
            arg1 = str(args[0]).replace('-', '').lower()
            arg2 = str(args[1]).replace('-', '').lower()
        else:
        ...
        if arg1 == 'all':
            ...
            if (len(args) == 1):
                ...
                if arg2 == 'all':
                    ...
                    #self.log.info(
                    #    '_parseargs: parse game for %s for date %s', arg2, date)
                    try:
        ...
        # Otherwise arg1 is a team, and we want today's game.
            ...
            if arg2:
                if arg2 == 'all':
                    ...
                    all_games = self._getscoreboardinfo(date)
                    result = self._printscore_all(all_games)
                elif __evaldate(arg2):
                    if (arg2 in ['yesterday', 'tomorrow']):
                        date = self._getdate(arg2)
                    else:
                        date = arg2[:4] + ' ' + arg2[4:6] + ' ' + arg2[6:9]
                    all_games = self._getscoreboardinfo(date)
                    if (len(args) == 1):
                        result = self._printscore_all(all_games)
                    elif (len(args) == 2):
                        ...
                        if arg1 == 'all':
                            #self.log.info(
                            #    '_parseargs: parse all games for %s', date)
                            result = self._printscore_all(all_games)
                        else:
                            #self.log.info(
                            #    '_parseargs: parse game for %s for date %s', arg1, date)
                            try:
                                search = self._searchgame(arg1, all_games)
                                result = self._procsearchresult(
                                    search, all_games)
                            except:
                                result = 'ERROR: No gamedata found.'
                            return result, date
    ...
    def _parseargsstandings(self, args):
        ...
        if (len(args) == 2):
            ...
            if (arg1 == 'all'):
                ...
                return result
            #self.log.info(
            #    '_parseargsstandings: arg1: %s self.leagues: %s', arg1, type(self.leagues))
    ...
    def mlb(self, irc, msg, args):
        ...
        if len(args) == 0:
            date = self._getdate()
            game_date = ircutils.bold(datetime.strptime(date, '%Y %m %d').strftime('%m/%d/%y') + ': ')
            try:
                results, _ = self._parseargsmlb('all')
            except:
                results = self._parseargsmlb('all')
            #print(game_date)
            if (type(results) == list):
                for l in results:
                    irc.reply(game_date + l)
                return
            else:
                irc.reply(game_date + results)
                return
        else:
            if len(args) > 2:
                irc.reply("ERROR: Too many arguments. See @help mlb")
                return
            results, date = self._parseargsmlb(args)
            #print(date)
            game_date = ircutils.bold(datetime.strptime(date, '%Y %m %d').strftime('%m/%d/%y') + ': ')
            if (type(results) == list):
                for l in results:
